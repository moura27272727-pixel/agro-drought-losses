# Закрываем главное ограничение: считаем климат не по всей РФ, а только по
# зерновому поясу (там сигнал не размыт Сибирью). Берём сеточные данные GPCC и
# GHCN-CAMS (NOAA PSL), режем по боксу и сравниваем с национальным климатом.
import json
import os
import numpy as np
import pandas as pd
...
# зерновой пояс: европ. РФ + Поволжье + Юж. Урал (туда пришлась засуха 2010)
...
def ensure_grids():
    # качаем сеточные файлы если их ещё нет (они большие, в гит не кладём)
    import subprocess
...
def find_coord(ds, *cands):
    # ищем как называется координата (lat или latitude и т.п.)
    for c in cands:
...
def belt_series(path, var, box):
    # средний по боксу месячный ряд переменной var
    ...
    lat = ds[latn].values
    la0, la1 = box["lat"]
    lat_sl = slice(la1, la0) if lat[0] > lat[-1] else slice(la0, la1)   # широта бывает по убыванию
    da = da.sel({latn: lat_sl, lonn: slice(*box["lon"])})
    w = np.cos(np.deg2rad(da[latn]))         # вес по широте (ячейки у полюсов меньше)
...
def main():
    ...
    if ta["air"].max() > 100:        # GHCN-CAMS в Кельвинах, переводим в Цельсии
    ...
    # национальный климат - из готового ряда по России
    ...
    # Рис.9: слева бары (нац. vs пояс), справа диаграмма рассеяния по поясу
